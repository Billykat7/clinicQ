...
```mermaid
flowchart LR
  A[Stage 0: 1 pilot clinic, dashboard+display proven] -->|30%+ remote joins, testimonial| B[Stage 1: 2-5 clinics]
  B -->|Positive P&L per clinic, support load manageable| C[Stage 2: 6-20 clinics, one metro]
  C -->|Install/support process documented| D[Stage 3: multi-metro / district deals]
  D -->|District/provincial MOU or practice-group rollout| E[Regional footprint]
```
