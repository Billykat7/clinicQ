...
| | |
|---|---|
| **Status** | 🚧 In progress: issues 48 (the dashboard shell, role-aware navigation and the site switcher), 49 (the live front-desk board), 52 (reordering with reason codes and the override trail), 53 (the nurse room view and private visit notes) and 54 (the clinic manager's settings screens) delivered |
