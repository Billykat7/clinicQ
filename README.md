...
| **[Milestones & issues](docs/GITHUB/README.md)** | 15 milestones, 123 tracked issues, conventions, release tags, pipeline strategy |
...
| 8 | [Display Monitor](docs/GITHUB/MILESTONES/M8_display_monitor.md) | 56–62, 237, 240 | 9 | `v0.8.0` | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 **100%** (9/9 issues) |
...
| 14 | [Production, Pilot & Go-live](docs/GITHUB/MILESTONES/M14_production_pilot_golive.md) | 102–109, 197, 239, 243 | 13–14 | `v0.14.0` | 🟩🟩🟩⬜⬜⬜⬜⬜⬜⬜ **27%** (3/11 issues) |
...
| ⭐ | **First official release:** every tracked issue closed | - | end of 14 | **`v1.0.0`** | 🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜ **76%** (94/124 issues) |
...
The bars in [Delivery at a glance](#delivery-at-a-glance) are the status: they are written by
`make milestone-progress` from **GitHub's own issue states**, in the pull request that closes each
issue, so they cannot drift from the work. `make milestone-progress-check` fails CI when they do.
What follows is only what a bar cannot say.

**Where the product is.** Eleven of fifteen milestones are finished. A patient can find a clinic,
join its queue from a phone, follow the ticket live and be told when to come in; staff run the whole
day from the dashboard; the waiting room has its board. Appointments, check-in and the patient-care
extras (M11) are in. Clinic onboarding and patient sign-in by email (M15) are the most recent.

**What is left.** The channels that reach a phone with no data (M10: USSD and WhatsApp), the reports
(M12), the security, privacy and POPIA work that has to be done before real patient data exists
(M13), and production, the pilot and go-live (M14).

**Tags.** `v0.1.0`–`v0.9.0` and `v0.11.0` are cut, each with its
[release note](docs/GITHUB/RELEASES/). `v0.10.0` waits for M10, and `v0.15.0` for this milestone's
last issue. A tag publishes an image; **deploying it is a separate, manual decision**
([runbook](docs/CICD/RUNBOOK_DEPLOY.md)).

| | Milestone |
|---|---|
| ✅ | Repo, local CI, and the pipeline running (M1–M2) |
| ✅ | Identity, RBAC, site scoping, audit and consent (M3) |
| ✅ | Clinics and queues configurable (M4) |
| ✅ | Patients find nearby clinics by GPS or suburb (M5) |
| ✅ | Queue engine running (M6) ⚠️ critical path |
| ✅ | Staff run the day from the clinic dashboard (M7) |
| ✅ | **MVP complete** — the waiting-room display (M8) |
| ✅ | Patients told, and following their ticket (M9) |
| ✅ | Appointments, check-in and patient-care extras (M11) |
| ✅ | Clinic onboarding and patient sign-in by email (M15) |
| ⬜ | USSD and WhatsApp (M10) · Reports (M12) · POPIA and hardening (M13) |
| ⬜ | Pilot clinic live, and the capstone deliverables (M14) |
