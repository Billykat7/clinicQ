...
| 6 | [**Queue Engine Core**](docs/GITHUB/MILESTONES/M6_queue_engine_core.md) ⚠️ critical path | 39–47 | 6–7 | `v0.6.0` | 🟩🟩🟩🟩🟩🟩🟩🟩🟩⬜ **89%** (8/9 issues) |
...
| ⭐ | **First official release:** every issue 1–109 closed | - | end of 14 | **`v1.0.0`** | 🟩🟩🟩🟩⬜⬜⬜⬜⬜⬜ **42%** (46/109 issues) |
...
**Phase:** building. **Sprints 1–5 of 14 complete**, sprints 6 and 7 under way (semester 2). **46 of 109
issues closed**. `v0.2.0` is the only tag cut so far; the notes for `v0.1.0` and `v0.3.0`–`v0.5.0` are
written and their tags are still to cut, in order. M6, the queue engine, is under way: tickets and their numbering (issue 39) the one join service every channel calls (issue 40) the ticket lifecycle (issue 41) honest wait ranges (issue 42) recall and no-show timers (issue 43) cancellation from any channel (issue 44) transfers that keep the visit (issue 45) and audited priority overrides (issue 46) are in.
